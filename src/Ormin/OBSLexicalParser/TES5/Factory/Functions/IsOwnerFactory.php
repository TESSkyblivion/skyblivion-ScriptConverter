...
class IsOwnerFactory implements FunctionFactory
{
    ...
    public function convertFunction(TES5Referencer $calledOn, TES4Function $function, TES5CodeScope $codeScope, TES5GlobalScope $globalScope, TES5MultipleScriptsScope $multipleScriptsScope)
    {
        ...
        $targetReference = $this->referenceFactory->createReadReference($functionArguments->getValue(0)->getData(), $globalScope, $multipleScriptsScope, $localScope);
        $datatype = ESMAnalyzer::instance()->getFormTypeByEDID($functionArguments->getValue(0)->getData());
        if ($datatype == TES5BasicType::T_FACTION()) {
            $owner = $this->objectCallFactory->createObjectCall(
            ...
            $base = $this->objectCallFactory->createObjectCall(
