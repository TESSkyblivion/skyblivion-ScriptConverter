...
class GetInSameCellFactory implements FunctionFactory
{
    ...
    public function convertFunction(TES5Referencer $calledOn, TES4Function $function, TES5CodeScope $codeScope, TES5GlobalScope $globalScope, TES5MultipleScriptsScope $multipleScriptsScope)
    {
        ...
        $functionThis = $this->objectCallFactory->createObjectCall(
            $calledOn, "GetParentCell", $multipleScriptsScope
        );

        $functionArgument = $this->objectCallFactory->createObjectCall(
            $this->referenceFactory->createReadReference($functionArguments->getValue(0)->getData(), $globalScope, $multipleScriptsScope, $localScope), "GetParentCell", $multipleScriptsScope
        );
