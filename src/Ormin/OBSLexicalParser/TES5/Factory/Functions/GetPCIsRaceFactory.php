...
class GetPCIsRaceFactory implements FunctionFactory
{
    ...
    public function convertFunction(TES5Referencer $calledOn, TES4Function $function, TES5CodeScope $codeScope, TES5GlobalScope $globalScope, TES5MultipleScriptsScope $multipleScriptsScope)
    {
        ...
        $race = $this->objectCallFactory->createObjectCall(
            $this->referenceFactory->createReferenceToPlayer(), "GetRace", $multipleScriptsScope
        );

        $checkAgainst = $this->referenceFactory->createReadReference($functionArguments->getValue(0)->getData(), $globalScope, $multipleScriptsScope, $localScope);
