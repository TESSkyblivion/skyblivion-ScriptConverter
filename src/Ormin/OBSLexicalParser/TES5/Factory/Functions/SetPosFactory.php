...
class SetPosFactory implements FunctionFactory
{
    ...
    public function convertFunction(TES5Referencer $calledOn, TES4Function $function, TES5CodeScope $codeScope, TES5GlobalScope $globalScope, TES5MultipleScriptsScope $multipleScriptsScope)
    {
        ...
        $dummyX = $this->objectCallFactory->createObjectCall($calledOn, "GetPositionX", $multipleScriptsScope);
        $dummyY = $this->objectCallFactory->createObjectCall($calledOn, "GetPositionY", $multipleScriptsScope);
        $dummyZ = $this->objectCallFactory->createObjectCall($calledOn, "GetPositionZ", $multipleScriptsScope);

        $argList = [];
        switch (strtolower($functionArguments->getValue(0)->getData())) {

            case 'x': {
                $argList[] = $this->valueFactory->createValue($functionArguments->getValue(1), $codeScope, $globalScope, $multipleScriptsScope);
                $argList[] = $dummyY;
                $argList[] = $dummyZ;
                break;
            }
        ...
        return $this->objectCallFactory->createObjectCall(
