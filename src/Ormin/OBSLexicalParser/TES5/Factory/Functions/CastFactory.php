...
class CastFactory implements FunctionFactory
{
    ...
    public function convertFunction(TES5Referencer $calledOn, TES4Function $function, TES5CodeScope $codeScope, TES5GlobalScope $globalScope, TES5MultipleScriptsScope $multipleScriptsScope)
    {
        ...
        $calledOn = $this->referenceFactory->createReadReference($value->getData(), $globalScope, $multipleScriptsScope, $localScope);
        ...
        if ($target !== null) {
            $methodArguments->add($this->referenceFactory->createReadReference($target->getData(), $globalScope, $multipleScriptsScope, $localScope));
        }
